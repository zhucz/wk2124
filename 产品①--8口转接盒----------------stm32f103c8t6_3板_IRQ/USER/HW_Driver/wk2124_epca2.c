...
void HW_WK2124_epca2_Init(void)
{
	...
	/*子串口 1 控制寄存器 */
	EXHW_WK2412S2_Write_Reg(SPAGE0_SCR(0),0x03);	//子串口1 发送使能  接收使能

	/*子串口 1 配置寄存器*/
	EXHW_WK2412S2_Write_Reg(SPAGE0_LCR(0),0x00);	//子串口1 正常输出,普通模式,8位数据位,0校验,1位停止位

	/*子串口 1 FIFO控制寄存器*/
	EXHW_WK2412S2_Write_Reg(SPAGE0_FCR(0),0x0F);	//子串口1 发送触发点,接收触发点 
																								//使能 发送,接收FIFO 复位发送接收FIFO
	/*子串口 1 中断使能寄存器*/
	EXHW_WK2412S2_Write_Reg(SPAGE0_SIER(0),0x83); //子串口1 使能接收FIFO数据错误中断
																								//禁止发送FIFO空中断
																								//禁止发送FIFO触点中断
																								//使能接收FIFO接收超时中断
																								//使能接收FIFO接收触点中断
...
void WK2124_epca2_IRQHandler(void)
{
    ...
    volatile uint16_t epca2_recv_cnt = 0;
		...
		if(epca2_irq_stat & (3 << 0)){//子串口 1 接收FIFO触点中断标志 ， 子串口 1 接收FIFO超时中断标志
      ...
      if(epca2.count > 498){
		...
		if(epca2_irq_stat & (1 << 7)){//子串口 1 接收FIFO数据错误中断标志
		
		}
		
		if(epca2_irq_stat & (1 << 2)){//子串口 1 发送FIFO触点中断标志
		
		}
		if(epca2_irq_stat & (1 << 3)){//子串口 1 发送FIFO空中断标志
...
uint16_t Wk2124S2_5_SendBuf(uint8_t *sendbuf,uint16_t len)
{
	...
	{
		tfcnt = EXHW_WK2412S2_Read_Reg(SPAGE0_TFCNT(0));//读子串口发送fifo中数据个数
...
uint16_t Wk2124S2_5_GetBuf(uint8_t *getbuf)
{
	...
	{
		rfcnt = EXHW_WK2412S2_Read_Reg(SPAGE0_RFCNT(0));//读子串口发送fifo中数据个数
